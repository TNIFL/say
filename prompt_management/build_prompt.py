...
def build_prompt(
    ...
    def pick_category(sel):
        if isinstance(sel, list):
            for c in sel:
                if c in category_templates.CATEGORY_GUIDE_MAP:
                    return c
            return "general"
        return sel if sel in category_templates.CATEGORY_GUIDE_MAP else "general"

    category_key = pick_category(selected_categories)
    user_guide = category_templates.CATEGORY_GUIDE_MAP[category_key]
